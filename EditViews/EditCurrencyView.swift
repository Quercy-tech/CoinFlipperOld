        ...
        NavigationView {
            Form {
                Section(header: Text("Currency")) {
                    ...
                    TextField("Currency rate", value: $selectedValue, format: .number)
...
struct EditCurrencyView_Previews: PreviewProvider {
    static var previews: some View {
        EditCurrencyView(currency: .constant(Currency(date: Date.now.formatted(), base: "UAH", rates: ["USD": 0.25])), CurrencyList: Currencies(), base: .constant("UAH"), codesAndNames: .constant(["USD": "Ukrainian Hryvnia"]), newFullName: "Ukrainian Hryvnia")
    }
}
